...
export async function getAllEventSheetData(): Promise<EventItem[]> {
  try {
    ...
    const rows: (string | number)[][] = dataResponse.values || [];
...
export function useEventSheetData(eventIdx: number) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const currentEvent = data[eventIdx];
      setEventItem(currentEvent);

      if (currentEvent.google_maps_link) {
        const query = extractGoogleMapsLink(currentEvent.google_maps_link);
        const url = await getGoogleMapsUrl(query);
        setMapUrl(url);
      }
